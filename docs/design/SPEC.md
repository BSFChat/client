...
- Full-screen button: full-screens **the video on the stage**, not the app window. It opens the dedicated fullscreen window described in §3.4 and the main window does not move, resize or change state. It used to write `Window.window.visibility = Window.FullScreen`, which covered the screen with the whole app — sidebar, channel list, member strip, dock — and left the video in its little panel in the middle; `tests/test_qml_hygiene.cpp` fails if a write to another window's `visibility` comes back to any voice-room video file.
