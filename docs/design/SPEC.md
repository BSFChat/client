...
- Full-screen button: full-screens **one video**, not the app window — the expanded feed when one is expanded, otherwise the feed the pointer is over, otherwise the first feed in the grid (`VideoStage.focusKeyFor()`). `F` does the same thing while the video stage has focus. It opens the dedicated fullscreen window described in §3.4 and the main window does not move, resize or change state. It used to write `Window.window.visibility = Window.FullScreen`, which covered the screen with the whole app — sidebar, channel list, member strip, dock — and left the video in its little panel in the middle; `tests/test_qml_hygiene.cpp` fails if a write to another window's `visibility` comes back to any voice-room video file.

---

### 3.4 Video stage (main content, alternate)
**Source:** `qml/components/VoiceRoom.qml` + `qml/components/VideoFeedTile.qml` + `qml/components/VideoPopoutWindow.qml` + `qml/components/VideoFullscreenWindow.qml`
**Policy:** `qml/js/VideoStage.js` (tested by `tests/qml/tst_videostage.qml`) and `qml/js/VideoWindows.js` (tested by `tests/qml/tst_videowindows.qml`)
...
**Stage — a grid by default, for any mix of feeds.** Screens and cameras alike go in one grid, everybody visible at once, self included, each tile as large as the stage allows. **Nothing is promoted to the stage on its own**, and a screen share is not special: it used to seize the stage the moment it started, which swept every face in the call into a row of thumbnails without anyone asking. Each feed is aspect-fit in its cell; a short final row is centred.

The grid shape is *measured*, not looked up. "As big as possible" is not "as few cells as possible" — a feed is aspect-fit, so a cell of the wrong shape is mostly letterbox, and two feeds on a stage wider than 16:9 are biggest side by side while on a narrower stage they are biggest stacked. For each column count 1…n (rows follows as `ceil(n/cols)`) `VideoStage.gridDims()` fits a 16:9 rectangle into one cell and keeps the largest; ties go to the wider grid, because at exactly 16:9 the choice between 3×2 and 2×3 is a dead heat and faces read left to right. Gaps are not part of the choice — single-digit pixels against a stage of several hundred never flip an answer, and leaving them out keeps the shape a pure function of the stage's aspect, which is what makes it testable.

On a 16:9-or-wider stage that yields exactly the case-by-case shapes: 1 fills, 2 side by side, 3–4 as 2×2, 5–6 as 3×2, 7–9 as 3×3; past nine the columns grow again and every tile shrinks, which is the honest answer to a call that has outgrown the stage. On a portrait stage it yields the transpose, down to a single column when the stage is tall and narrow enough for that to genuinely win. `tests/qml/tst_videostage.qml` pins both lists *and* the rule behind them: for every shape it checks that no other column count would have given a bigger tile.

**Expanding one feed.** Clicking a tile expands it to fill the stage and pushes every other feed into the bottom strip — the layout you want when somebody is sharing a screen *and* on camera. Clicking the expanded feed again, pressing Escape, or clicking its "on stage" chip in the strip returns to the grid; clicking a *different* strip thumbnail swaps which feed is expanded. A feed that ends while expanded returns the view to the grid — it does **not** hand the stage to some other feed, because the user expanded that one and the next-best guess is a view they never asked for. Expansion otherwise survives feed membership changes, so a peer joining does not collapse what you are watching. A lone feed is always the grid: a 1×1 grid already fills the stage, and a strip holding one thumbnail of the only picture is a chip that can do nothing.

**Strip (92h, above the member strip):** exists **only while a feed is expanded** — in the grid every feed is already on the stage and a strip under it would be a second, smaller copy of the same pictures. It holds thumbnails of *all* feeds, including the expanded one, which appears as a highlighted, clickable "on stage" chip rather than a second copy of the picture. Thumbnails shrink to fit rather than scrolling, so nothing is unreachable — the strip is the way back to the grid other than Escape, and an unclickable thumbnail would be an unreachable feed.

**Keyboard.** ←/→ walk *which feed is expanded*, in feed order, clamped rather than wrapped. In the grid they are a no-op: every feed is already visible, so there is nothing to walk between, and a bare arrow key that suddenly expanded something would be a large unasked-for change of view. Escape collapses to the grid. `F` full-screens the expanded feed, or in the grid the feed under the pointer (falling back to the first). All of these are `Keys` handlers on the stage rather than `Shortcut`s: a window-wide Left/Right/F would fire inside every text field and settings dialog in the app.

**Tile chrome.** The hover actions (pop out, full screen) and the diagnostics overlay are drawn only on a tile with room for them — a nine-up grid in a small window makes tiles a couple of hundred pixels wide, and two 26px buttons plus a mono stats line over one of those is more chrome than picture. The right-click menu still reaches every action on every tile, so nothing becomes unreachable. The accent highlight means "this is the feed you expanded" and is therefore absent in the grid: ringing every tile in accent would say nothing at all.
...
**The constraint that shapes the code.** There is exactly one `VideoOutput` per feed *in the voice room*, declared only in `VideoFeedTile.qml`, and it serves both positions. (The pop-out and fullscreen windows below each declare one of their own, on purpose: they are additional sinks on the same stream, not the room's tile moved.) `VoiceRoom` keeps a single `Repeater` over the feed list and moves tiles by writing `x`/`y`/`width`/`height`. Do not reparent a tile, do not split stage and strip into two Repeaters, and do not put liveness on the feed objects: any of those changes the delegate's identity, the engine destroys and rebuilds the `VideoOutput`, and the picture goes black until the next frame — a fifth of a second on a 5 fps share, on every click.
...
**Full screen (one feed, one whole screen).** The fullscreen button on a feed — header, tile hover, tile context menu, or `F` while the voice room has focus — opens `VideoFullscreenWindow.qml`: a dedicated top-level window, black, the feed aspect-fit, no app chrome. Leave with Escape, `F`, a double-click, or the exit button. The chrome (owner's name, screen/camera, exit) and the mouse cursor fade out after `VideoWindows.OVERLAY_IDLE_MS` of no movement and come back on the next one. It closes itself when its feed ends — the stream stops, the peer leaves, we leave the channel — because the Loader that holds it is bound to the feed still being in `_feeds`.

*macOS, and why the flags are what they are.* Qt has two full screens on macOS. A window carrying `Qt::WindowFullscreenButtonHint` gets the **native** one: its own Space, with the animation — wrong twice over, because the Space swipes away from the call and a window destroyed while native-fullscreen is what strands an empty Space in Mission Control. A window without that hint gets the **non-native** one: borderless, resized to the screen, menu bar and Dock hidden through `NSApplicationPresentationOptions`, staying in the current Space. So the window is declared `Qt.Window | Qt.FramelessWindowHint` with no fullscreen-button hint (a borderless `NSWindow` cannot have that button at all, which is what keeps Qt off the native path) and `transientParent: null` (a transient child is ordered with its parent and cannot own the screen). The hygiene test pins all three.

**Pop-out (one feed, one ordinary window).** Every feed has a pop-out action, on hover next to full screen and in the tile's context menu. It opens `VideoPopoutWindow.qml`: a normal resizable window titled `<name> — screen` / `<name> — camera`, the feed aspect-fit on black, with a keep-on-top pin and its own full-screen toggle (the same window as above — `VideoWindows.fullscreenOwner()` says a popped-out feed's full screen belongs to the pop-out, so the app can never stack two fullscreen windows of one feed on one screen). Default size is ~960×540's worth of *area* in the feed's aspect ratio, minimum 320×180, and the last size/position is remembered in `Settings` **per kind** rather than per feed — a feed key carries a user id, so per-feed memory would grow an entry for every person ever watched. A remembered position is refused if the monitor it names is no longer attached, and a second window restoring the same position cascades rather than landing exactly on top of the first.

Several pop-outs can be open at once, one per feed; asking again for a feed that already has one **raises** that window instead of opening a second copy of the same picture. A pop-out closes when its feed ends or when the user leaves the voice channel, and closing one never touches the call.

**And the tile stays live.** While a feed is popped out its in-room tile keeps running exactly where it is and grows a small "POPPED OUT" badge — nothing is reparented, hidden or detached, so the "no `VideoOutput` recreation on selection change" rule above is untouched. That is possible only because `VideoStreamRegistry` fans **one** decoded stream out to **every** attached sink: the tile, the pop-out and a fullscreen window can be three surfaces on one decode, `attachOutput` replays the last frame so a window opened mid-stream is never blank, a sink that is destroyed detaches itself, and a stream that stops blanks all of them together. `tests/test_video_registry.cpp` pins all four halves of that; our own camera and screen preview get the same fan-out from their controller's `forwardTo()`, which now replays the current frame on attach for the same reason.

**Policy:** `qml/js/VideoWindows.js` (tested by `tests/qml/tst_videowindows.qml`) — titles, default and remembered geometry, the cascade, pop-out open/raise/close-on-feed-end bookkeeping, which window owns a feed's full screen, the overlay idle rule, and the one copy of the three-way surface routing (our own preview mirrors its controller, a remote stream comes from the registry; a self-feed sent down the remote path is a permanently black window).
