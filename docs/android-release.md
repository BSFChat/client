...
**JDK 17 is required**, not the system default (which is 26 here). D8
throws `NullPointerException` while dexing anonymous inner classes
compiled by JDK 26 — which is how a perfectly healthy C++ build fails at
the very last step with no useful message. Qt 6.10 documents JDK 17 too,
so CI pins the same.
