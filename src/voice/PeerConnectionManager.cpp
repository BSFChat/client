...
// sends on `answererSsrc`. Declaring both in the offer is what makes
// the reverse direction work too — the answer is generated by
// reciprocating the offer's media (SSRC attributes and all) on a
// libdatachannel worker thread, long before our queued onTrack handler
// could add anything of its own to it.
//
